...
class Model(nn.Module):
    ...
    def train_batch(self, context, responses, index, sentinel, new_epoch, context_lengths, target_lengths, clip_grads,
        ...
        h = torch.cat((h_context, h_profile), dim=1)
...
class Decoder(nn.Module):
    ...
    def __init__(self, emb_size, hops, gru_size, nwords):
        ...
        def init_weights(m):
            if type(m) == torch.nn.Embedding:
                m.weight.data = torch.normal(0.0, torch.ones(self.nwords, 2*self.emb_size) * 0.1)

        self.A = torch.nn.ModuleList(
            [torch.nn.Embedding(self.nwords, 2*self.emb_size, padding_idx=0) for h in range(self.hops)])
        self.A.apply(init_weights)
        self.C = torch.nn.ModuleList(
            [torch.nn.Embedding(self.nwords, 2*self.emb_size, padding_idx=0) for h in range(self.hops)])
        self.C.apply(init_weights)
        ...
        self.lin_vocab = nn.Linear(4 * emb_size, self.nwords)
        self.gru = nn.GRU(2*emb_size, 2*emb_size, dropout=self.dropout)
    ...
    def load_memory(self, context):
        ...
        for hop in range(self.hops):
            ...
            m = m.view(size[0], size[1], size[2], 2*self.emb_size)
            m = torch.sum(m, 2)
            self.memories.append(m)
            c = self.C[hop](context)
            c = c.view(size[0], size[1], size[2], 2*self.emb_size)
            c = torch.sum(c, 2)

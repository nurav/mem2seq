...
class TextDataset(Dataset):
    ...
    def preprocess(self):
        ...
        for idx in range(len(self.memory)):
            ...
            self.processed_context[idx].append(new_prof_mem)

    def __len__(self):
        return len(self.memory)

    def __getitem__(self, idx):
        return self.processed_context[idx][0], self.processed_context[idx][1], self.memory[idx][2], self.memory[idx][3], \
               self.memory[idx][4], self.memory[idx][0], self.memory[idx][1], self.processed_context[idx][2], self.memory[idx][6]
    ...
    for i, x in enumerate(batch):
        ...
        return torch.from_numpy(out_context), torch.from_numpy(out_target), torch.from_numpy(out_index), torch.from_numpy(
            out_gate), context_lengths, target_lengths, out_dialog_idxs, context_words, target_words, \
               torch.from_numpy(out_prof_mem), profile_words
...
def read_dataset(string, kb_entries):
    ...
    for line in bytez.splitlines():
        if len(line) == 0:
            ...
            memory.append([context_new, bot, idx, sentinel, dialog_idx, profile_memory[-1], profile_memory_words])  ##### final output
